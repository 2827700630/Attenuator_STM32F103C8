...
/* MSPM0 类型定义 */
// GPIO 端口通常由 SysConfig 定义的宏表示 (例如 GPIOA, GPIOB)
// 引脚也由 SysConfig 定义的宏表示 (例如 GPIO_LE_PIN, GPIO_PS_PIN)
// 这里使用通用指针和 uint32_t 作为占位符，具体类型取决于 SysConfig 配置
...
typedef struct
{
    ...
    // 这里保留 port/pin 结构，但具体使用方式在 Init 函数中处理
...
/**
 * @brief 串行模式设置衰减值（0-31.5 dB，步进 0.5 dB）
 * @param Attenuator: 指向衰减器设备句柄的指针
 * @param attenuation: 目标衰减值（0.0, 0.5, 1.0, ..., 31.5）
 * @return: 操作状态 (Attenuator_Status)
 * @note 适用于 HMC624A/PE4302 等支持串行模式的衰减器
 */
